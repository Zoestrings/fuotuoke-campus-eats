...
export default function DashCart({ isStaff, accent, cart, addItem, removeItem, cartCount, cartTotal, outlet, setOutlet, orderType, setOType, placeOrder, setPage }) {
  const [faculty, setFaculty] = useState("");
  const isGold = isStaff;
    ...
    <div className="page-bg">
      <div className="page-content-small">
        ...
        <h2 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(1.4rem,3vw,1.9rem)", fontWeight: 900, color: "var(--text-dark)", marginBottom: 24, display: "flex", alignItems: "center", gap: 10 }}>
          <i className="bi bi-cart3" style={{ color: isGold ? "var(--gold)" : "var(--primary)" }} />
          Your Cart
        </h2>
        ...
        {cart.length === 0 ? (
          <div style={{ textAlign: "center", padding: "64px 20px", background: "#fff", borderRadius: 16, border: "1px solid var(--border)" }}>
            ...
            <Btn variant={isGold ? "gold" : "primary"} onClick={() => setPage("menu")}>
          ...
          <>
            ...
            <SectionCard>
              ...
              <div className="filters-row" style={{ marginBottom: 0 }}>
                {OUTLETS.map(o => (
                  <button
                    key={o.id}
                    className={`filter-btn${outlet === o.id ? (isGold ? " active-gold" : " active-primary") : ""}`}
              ...
              <div style={{ display: "flex", gap: 10 }}>
                {[
                  ...
                  <button
                    ...
                    style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 7, padding: "11px 0", border: `1.5px solid ${orderType === val ? (isGold ? "var(--gold)" : "var(--primary)") : "var(--border)"}`, borderRadius: 9, cursor: "pointer", fontWeight: 600, fontSize: ".84rem", background: orderType === val ? (isGold ? "var(--gold-light)" : "var(--primary-light)") : "#fff", color: orderType === val ? (isGold ? "var(--gold)" : "var(--primary)") : "var(--text-light)", transition: "all .15s" }}
                  >
            ...
            <SectionCard>
                ...
                <div key={item.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 0", borderBottom: "1px solid var(--border)" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div className="qty-adjuster">
                      <button
                        className="qty-btn"
                        style={{ width: 26, height: 26, border: `1.5px solid ${isGold ? "var(--gold)" : "var(--primary)"}`, color: isGold ? "var(--gold)" : "var(--primary)" }}
                      ...
                      <button
                        className={`qty-btn-filled${isGold ? " staff" : ""}`}
                    ...
                    <span style={{ fontWeight: 800, color: isGold ? "var(--gold)" : "var(--primary)", minWidth: 82, textAlign: "right", fontSize: ".9rem" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 16, fontWeight: 800, fontSize: "1rem", color: "var(--text-dark)" }}>
                ...
                <span style={{ color: isGold ? "var(--gold)" : "var(--primary)" }}>₦{cartTotal.toLocaleString()}</span>
              </div>
            </SectionCard>

            {/* Place order */}
            <Btn full variant={isGold ? "gold" : "primary"} onClick={handlePlaceOrder} style={{ padding: "14px 0", fontSize: ".95rem" }}>
