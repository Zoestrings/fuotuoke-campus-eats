...
export default function DashMenu({ isStaff, accent, outlet, setOutlet, orderType, setOType, cart, addItem, removeItem, cartCount, cartTotal, setPage, onCustomizeItem, menuItems = [] }) {
      ...
      <div className="page-content-menu">
        ...
        <div className="page-section-header">
          <h2 className="page-heading">
            <i className="bi bi-grid" style={{ color: isGold ? "var(--gold)" : "var(--primary)" }} />
            Campus Menu
          </h2>
        ...
        <div className="order-type-toggle">
          {[
            ...
            <button
              ...
              className={`order-type-btn${orderType === val ? (isGold ? " active-gold" : " active-primary") : ""}`}
            >
        ...
        <div className="search-cat-bar">
          <div className="search-input-wrapper">
            ...
            <input
              className={`search-input${isGold ? " staff" : ""}`}
          ...
          <div className="cat-filter-group">
            {CATS.map(c => (
              <button
                key={c}
                className={`filter-btn${cat === c ? (isGold ? " active-gold" : " active-primary") : ""}`}
                onClick={() => setCat(c)}
              >
                {c}
              </button>
        ...
        {menuItems.length === 0 ? (
          <div className="empty-state">
            ...
            <p style={{ fontSize: ".82rem", color: "var(--text-light)", marginBottom: 0 }}>
              No meals have been added yet. Please ask the Admin to add items from the Admin Panel.
            </p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="empty-state">
            ...
            {filtered.map(item => {
              ...
              return (
                <div
                  ...
                  className={`menu-card${isGold ? " staff" : ""}`}
                >
                  <div className="menu-card-top">
                    ...
                    {item.popular && (
                      <span className={`badge ${isGold ? "badge-gold" : "badge-blue"}`} style={{ height: "fit-content" }}>
                  ...
                  <div className="menu-card-footer" onClick={e => e.stopPropagation()}>
                    <span className="menu-card-price" style={{ color: isGold ? "var(--gold)" : "var(--primary)" }}>
                      ₦{item.price.toLocaleString()}
                    </span>
                    {qty === 0 ? (
                      <Btn sm variant={isGold ? "gold" : "primary"} onClick={e => { e.stopPropagation(); onCustomizeItem(item); }}>
        ...
        {cartCount > 0 && (
          <div className="floating-cart-wrap">
            <button
              ...
              className={`floating-cart-btn${isGold ? " staff" : ""}`}
            >
