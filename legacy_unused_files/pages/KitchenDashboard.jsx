...
export default function KitchenDashboard({ user, onLogout, menuItems = [], setMenuItems, orders, setOrders }) {
  ...
  const analyticsCards = [
    ...
    { label: "Active Orders",    icon: "bi-lightning-fill", value: `${activeOrders.length} active`,     sub: "Needs prep / dispatch",                         valueColor: "var(--gold)"                                    },
      ...
      <nav className="navbar staff">
        ...
        <div className="nav-links">
          ...
          <div className="nav-user-info">
            ...
            <span className="nav-user-role-badge" style={{ color: "var(--gold)", borderColor: "var(--gold)" }}>
              Kitchen Hub
            </span>
          </div>
      ...
      <div className="page-bg" style={{ paddingTop: 24 }}>
        <div className="page-content-vendor">
          ...
          <div className="tabs-header">
            <button className={`tab-btn${activeTab === "active" ? " active staff" : ""}`} onClick={() => setActiveTab("active")}>
              <i className="bi bi-clock-history" style={{ marginRight: 6 }} />
              Active Orders ({activeOrders.length})
            </button>
            <button className={`tab-btn${activeTab === "completed" ? " active staff" : ""}`} onClick={() => setActiveTab("completed")}>
              <i className="bi bi-check2-circle" style={{ marginRight: 6 }} />
              Completed ({completedOrders.length})
            </button>
            <button className={`tab-btn${activeTab === "menu" ? " active staff" : ""}`} onClick={() => setActiveTab("menu")}>
                      ...
                      <div key={order.id} className="order-card">
                        <div className="order-card-header">
                          ...
                          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                            ...
                            <Btn sm variant="gold" onClick={() => advance(order.id, status, order.type)}>
                ...
                <div className="vendor-items-list">
                  {menuItems.length === 0 ? (
                    <div className="empty-state" style={{ padding: "40px 20px", gridColumn: "1 / -1", textAlign: "center" }}>
                      ...
                      <Btn sm variant="gold" onClick={handleLoadDemo}>
                        <i className="bi bi-cloud-arrow-down-fill" style={{ marginRight: 6 }} /> Load Default Campus Meals
                      </Btn>
                    </div>
                  ) : (
                    menuItems.map(item => (
                      <div key={item.id} className="vendor-item-card staff">
                        <button
                          type="button"
