...
export default function VendorSignup({ onSignup, goLogin, goHome }) {
  ...
  const [canteen, setCanteen] = useState("Main Cafeteria");
  ...
  const handleSignupSubmit = () => {
    ...
    const idUpper = kitchenId.toUpperCase().trim();
    if (!/^FUO-KIT-\d{4}$/.test(idUpper)) {
      setError("Kitchen ID format: FUO-KIT-0000 (e.g. FUO-KIT-0042)");
      return;
    }
  ...
  return (
    <AuthShell title="Register Kitchen Staff" subtitle="Join FUOTUOKE Canteen Hub." accent="gold">
      <div className="auth-name-row">
        <Field label="First Name" placeholder="e.g. Grace" value={firstName} onChange={e => setFirst(e.target.value)} />
        <Field label="Surname" placeholder="e.g. Adeyemi" value={lastName} onChange={e => setLast(e.target.value)} />
      </div>

      <Field label="Email Address" type="email" placeholder="canteen@fuotuoke.edu.ng" value={email} onChange={e => setEmail(e.target.value)} />
      
      <div className="form-group" style={{ marginBottom: 16 }}>
        <label className="form-label">Select Canteen Outlet <span className="auth-required">*</span></label>
        <select className="form-input" value={canteen} onChange={e => setCanteen(e.target.value)} style={{ width: "100%", cursor: "pointer" }}>
          {OUTLETS.map(o => (
      ...
      <Field 
        label="Kitchen ID" 
        placeholder="e.g. FUO-KIT-0042" 
        value={kitchenId} 
        onChange={e => setKitchenId(e.target.value)} 
        hint="Format: FUO-KIT-0000" 
      />
      ...
      <Btn full variant="gold" onClick={handleSignupSubmit} className="auth-submit-btn">
        <i className="bi bi-person-check-fill" /> Register Canteen
      </Btn>
